...
public class AVSLogin extends CordovaPlugin
{
    ...
    public boolean execute(String action, CordovaArgs args, CallbackContext callbackContext) throws JSONException
    {
        ...
        if (action.equals(ACTION_AUTHORIZE))
        {
            ...
            CurrScope = Params.optString("scope", "alexa:all");
            CurrScopeData = Params.optJSONObject("scope_data");
    
            if (CurrScope.equals("alexa:all") && ! CurrScopeData)
            {
    ...
    private final Runnable GetToken = new Runnable(){
        ...
        @Override
        public void run() {
            AuthorizationManager.getToken(this, new Scope[] { ScopeFactory.scopeNamed("alexa:all")}, new Listener<AuthorizeResult, AuthError>() {
                @Override
                public void onSuccess(AuthorizeResult result) {
